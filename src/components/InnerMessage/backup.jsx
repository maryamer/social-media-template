...
export default function InnerMessage() {
  ...
  return (
    <>
      <div className="flex flex-col h-screen md:h-full justify-end">
        <InnerMessageHeader item={item} />
        <InnerMessages />
        <MessageInput />
      </div>
    </>
...
function InnerMessageHeader({ item }) {
  return (
    <div
      className="messageHeader border-b border-b-slate-500 flex  group justify-between items-center h-[10%]
      p-3"
    >
      <div className="flex items-center hover:text-slate-400">
        <img
          ...
          alt={item.username}
          className="shareProfileImg  cursor-pointer object-cover flex items-center shrink-0 grow-0 md:w-14 md:h-14  w-12 h-12 rounded-full"
        />
        &nbsp; &nbsp;
        <CharaterName item={item} />
      </div>
      <IoMdSettings className="hover:text-slate-600 hover:cursor-pointer xl:mr-2.5 lg:mr-1.5 xl:w-10 xl:h-10 w-8 h-8   text-slate-400" />
    </div>
  );
}
...
function MessageInput() {
  return (
    <div class="flex items-center justify-center w-full h-[10%] ">
      <div class="bg-gray-50 w-[95%] lg:w-[97%] border  border-gray-300 text-gray-900 text-sm rounded-3xl  flex items-center justify-around  p-0  lg:p-1.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white ">
